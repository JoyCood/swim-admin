...
class SwimAdmin {
    ...
    static public function pgAggregate($url, $collection, $pn=1, $filters=array(), $sort=array(), $limit=null) {
        ...
        $count = empty($count['cursor']['firstBatch'][0]['count'])? 0: $count['cursor']['firstBatch'][0]['count'];
        $pn    = intval($pn);
        $pn    = $pn < 0? 1: $pn;
        $sort  = empty($sort)? array('create_time' => -1): $sort;
        $limit = $limit? $limit: Config::$admin['listRowsNum'];
        $skip  = ($pn - 1) * $limit;
        $filters[] = array('$sort'  => $sort);
        $filters[] = array('$skip'  => $skip);
        $filters[] = array('$limit' => $limit);
		$result = $collection->aggregate(array_values($filters), ['cursor'=> ['batchSize'=>$limit]]);
		$url 	 = self::setURIPageNumber($url? $url: $_SERVER['REQUEST_URI'], '');
        return array(
            'total' => $count,
            'pn'    => $pn,
            'pages' => ceil($count / $limit),
            'rows'  => count($result['cursor']['firstBatch']),
    ...
    static public function elastic() {
        if(!is_object(self::$elastic)) {
            require_once(LIB_DIR. 'elasticsearch/vendor/autoload.php');     
            $hosts = [Config::$elastic['host']];
            self::$elastic = Elasticsearch\ClientBuilder::create()->setHosts($hosts)->build();
        } 
        return self::$elastic;
    }

    static public function predis() {
        if(!is_object(self::$predis)) {
            require_once(LIB_DIR . 'predis/autoload.php');
            ...
            self::$predis = new Predis\Client($setting);
        }
        return self::$predis;
    }
