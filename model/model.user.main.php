...
class ModelUserMain {
    ...
    static $privileges = array(
        ...
        self::USER_TYPE_SHENHUXI => array(
            'privilege' => array(
                'users' => array(PERM_READ, PERM_ADD, PERM_EDIT, PERM_DELETE),
                'orders' => array(PERM_READ, PERM_ADD, PERM_EDIT, PERM_REFUND, PERM_ALLOCATE_TEACHER, PERM_ALLOCATE_YONGGUAN, PERM_NOTES)
            ),
            'options' => array(
        ...
        self::USER_TYPE_CAIWU => array(
            'text' => '财务',
            'privilege' => array(
                'orders' => array(PERM_READ, PERM_REFUND, PERM_FINANCE_READ, PERM_FINANCE_EDIT, PERM_FINANCE_EXPORT),
            ) 
        )
    );
	...
	private $fields = array(
		...
		'type'          => 0,       // 帐户类型：1商户
        'privilege'     => 0,       // 权限
		'natatorium_id' => 0,       // 泳馆ID（如果此用户类型为泳馆的情况下）
		'teacher_id'    => 0,       // 教练ID (如果此用户类型为教练的情况下)
	);
