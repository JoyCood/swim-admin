...
$app->get('/interview/list.html', function() use($app) {
    ...
    $tags = SwimAdmin::model('interview.tag')->find(array('status'=>1));
...
$app->get('/interview/form.html', function() use($app) {
	...
	$category = SwimAdmin::model('interview.category');
	$dataCats = array();
    ...
    $Tag = SwimAdmin::model('interview.tag');
    $dataTags = array();
    if(isset($data['tags'])) {
        foreach($data['tags'] as $tag) {
            $tid = $tag['id'];
            $dataTags[$tid] = $tag['name'];
        }
    }
    $filter = array('status'=>1);
    $tagList = $Tag->find($filter);
    $tags = array();
    foreach($tagList as $tag) {
        $tid = (string)$tag['_id']; 
        $tag['_id'] = $tid;
        $tag['selected'] = isset($dataTags[$tid]);
        $tags[] = $tag;
    }
...
$app->post('/interview/form.html', function() use($app) {
    ...
    $Tag  = SwimAdmin::model('interview.tag');
    $tags = $app->request->post('tags', array());
    $dataTags = array(); 
    foreach($tags as $tag) {
        $tmp   = explode(':', $tag);
        $tid   = $tmp[0];
...
$app->get('/interview/tags.html', function() use($app){
    SwimAdmin::checkPermission('main-contents', PERM_READ);
    $Tag = SwimAdmin::model('interview.tag');
    $tags = $Tag->find();
    $app->view()->addGlobal('_MOD', 'discover');
    $app->render('interview/tag-index.html', array(
        'tags' => $tags
    ));
});

$app->get('/interview/tag/form.html', function() use($app){
    SwimAdmin::checkPermission('main-contents', PERM_READ);
    $id   = trim($app->request->get('id'));
    $Tag  = SwimAdmin::model('interview.tag');
    $data = array();
    if($id) {
        $data = $Tag->findOne(array('_id' => new MongoId($id))); 
    }
    $app->render('interview/tag-form.html', array(
       'tag' => $data 
    ));
});

$app->post('/interview/tag/form.html', function() use($app){
    ...
    if(!$id) {
        ...
        $Tag->update($filter, $data);
    }
    $app->render('interview/tag-list.html', array(
        'tags' => $Tag->find()
    ));
});
