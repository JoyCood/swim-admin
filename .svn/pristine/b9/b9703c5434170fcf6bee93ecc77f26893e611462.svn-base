...
class ControlUser {
	...
	public function formAction() {
		...
		if(empty($id)) {
			...
			if(!empty($data['type']) && $data['type']==$user::USER_TYPE_JIAOLIAN) {
			    $teacher = SwimAdmin::model('teacher.main')->findOneById($data['teacher_id']);
			}else if(!empty($data['type']) && $data['type']==$user::USER_TYPE_YONGGUAN) {
				$filters = array('_id' => new MongoId($data['natatorium_id']));
		        $teacher = SwimAdmin::model('natatorium.main')->findOne($filters);	
			}
        }
